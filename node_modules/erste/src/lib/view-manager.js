...
class ViewManager {
    ...
    pull(view, opt_canGoBack) {
        ...
        const pullAnimation = (timestamp) => {
        if (!start) start = timestamp
            const progress = timestamp - start
            const animationDuration = 350
            const alpha = (animationDuration - progress) / (animationDuration * 2)
            ...
            view.el.style['boxShadow'] = `-100vw 0 0 rgba(0, 0, 0, ${alpha})`;
            
            if (progress < animationDuration) {
                window.requestAnimationFrame(pullAnimation)
            }
        }
    ...
    push() {
        ...
        window.requestAnimationFrame(() => {
            ...
            window.requestAnimationFrame(pushAnimation);
        });
        let start = null

        const pushAnimation = (timestamp) => {
        if (!start) start = timestamp
            const progress = timestamp - start
            const animationDuration = 350
            const alpha = (animationDuration - progress) / (animationDuration * 2)
            ...
            currentView.el.style['boxShadow'] = `-100vw 0 0 rgba(0, 0, 0, ${alpha})`;
    ...
    backGestureTouchMove_(e) {
        ...
        var boxShadowAlpha = Math.floor(math.lerp(1, 0, currentViewDiff / (viewWidth - this.firstX_)) * 100) / 200;
        ...
        window.requestAnimationFrame(() => {
            ...
            currentView.el.style['boxShadow'] = `-100vw 0 0 rgba(0, 0, 0, ${boxShadowAlpha})`;
        });
    };
